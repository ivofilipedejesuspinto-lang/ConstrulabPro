...
export const VolumeMaterials: React.FC<VolumeMaterialsProps> = ({ unitSystem, importedAreaM2, isPro, user, onRequestUpgrade }) => {
  ...
  useEffect(() => {
    // If area imported, logic handled in render/calculation
  }, [importedAreaM2, mode]);
  ...
  const steelBreakdown = useMemo(() => {
      ...
      if (mode === 'slab') {
          return {
              type: 'Laje (Malha)',
              parts: [
                  { name: 'Malha Principal (Inf/Sup)', detail: 'Ø10 ou Ø12 // 15-20cm', weight: avgSteel * 0.6 },
                  { name: 'Distribuição / Cavaletes', detail: 'Ø8 // 20-25cm', weight: avgSteel * 0.4 }
              ]
          };
      } else {
          return {
              type: 'Viga/Pilar',
              parts: [
                  { name: 'Longitudinal (Varões)', detail: '4x a 8x Ø12 a Ø20', weight: avgSteel * 0.7 },
                  { name: 'Estribos (Cinta)', detail: 'Ø6 ou Ø8 // 15cm', weight: avgSteel * 0.3 }
              ]
          };
      }
      ...
      <div className="bg-slate-900 rounded-2xl border border-slate-800 shadow-2xl ring-1 ring-white/5 overflow-hidden flex flex-col">
        ...
        <div className="bg-slate-900/50 backdrop-blur-md border-b border-slate-800/60 p-5 flex justify-between items-center">
          <div className="flex items-center gap-4">
             ...
             <div>
               <h2 className="text-base font-bold text-slate-100 tracking-wide uppercase">Dimensões</h2>
               <div className="text-xs text-slate-500 font-medium mt-0.5">Definir volume da estrutura</div>
             </div>
          </div>
          
          <div className="flex bg-slate-950 p-1.5 rounded-xl border border-slate-800">
             <button onClick={() => setMode('slab')} type="button" className={`px-4 py-2 text-xs font-bold uppercase tracking-wide rounded-lg transition-all ${mode === 'slab' ? 'bg-indigo-600 text-white shadow-md' : 'text-slate-500 hover:text-slate-300'}`}>Laje</button>
             <button onClick={() => setMode('box')} type="button" className={`px-4 py-2 text-xs font-bold uppercase tracking-wide rounded-lg transition-all ${mode === 'box' ? 'bg-indigo-600 text-white shadow-md' : 'text-slate-500 hover:text-slate-300'}`}>Caixa</button>
          </div>
        </div>

        <div className="p-8 space-y-8 flex-1 flex flex-col">
          {mode === 'slab' && importedAreaM2 > 0 ? (
            <div className="p-8 bg-gradient-to-br from-indigo-900/20 to-blue-900/10 border border-indigo-500/30 rounded-2xl relative overflow-hidden">
              <div className="absolute top-0 right-0 p-6 opacity-10"><Scan size={96} className="text-indigo-400"/></div>
              <div className="text-sm font-bold text-indigo-300 mb-2 uppercase tracking-wider">Área do Canvas</div>
              <div className="text-5xl font-mono text-white tracking-tight font-bold">
                {formatNumber(convertValue(importedAreaM2, 'area', unitSystem))} <span className="text-2xl text-indigo-400 opacity-70">{units.area}</span>
              </div>
            </div>
          ) : (
             <div className="grid grid-cols-2 gap-6">
              <div className="group">
                <label className="block text-xs font-bold text-slate-500 mb-2 uppercase tracking-wide group-focus-within:text-indigo-400 transition-colors">Comprimento ({units.length})</label>
                <div className="relative">
                  <input type="number" value={length} onChange={e => setLength(e.target.value)} className="w-full bg-slate-950 border border-slate-800 rounded-xl p-4 text-lg text-white focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500/50 outline-none transition-all font-mono placeholder-slate-700"/>
                </div>
              </div>
              <div className="group">
                <label className="block text-xs font-bold text-slate-500 mb-2 uppercase tracking-wide group-focus-within:text-indigo-400 transition-colors">Largura ({units.length})</label>
                <input type="number" value={width} onChange={e => setWidth(e.target.value)} className="w-full bg-slate-950 border border-slate-800 rounded-xl p-4 text-lg text-white focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500/50 outline-none transition-all font-mono placeholder-slate-700"/>
              </div>
            </div>
          )}
          ...
          <div className="group">
            <label className="block text-xs font-bold text-slate-500 mb-2 uppercase tracking-wide group-focus-within:text-indigo-400 transition-colors">
              {mode === 'slab' ? `Espessura (${units.length})` : `Altura (${units.length})`}
            </label>
            <input type="number" value={height} onChange={e => setHeight(e.target.value)} className="w-full bg-slate-950 border border-slate-800 rounded-xl p-4 text-lg text-white focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500/50 outline-none transition-all font-mono placeholder-slate-700" step="0.05"/>
          </div>
          ...
          <div className="mt-auto pt-8 border-t border-slate-800/60">
             <div className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-3 flex items-center gap-2">
               <Container size={16}/> Volume Estimado
             </div>
             <div className="text-6xl font-bold text-emerald-400 font-mono tracking-tight flex items-baseline gap-3">
               {formatNumber(displayVolume, 3)} <span className="text-2xl text-emerald-600/80 font-sans font-bold">{units.volume}</span>
             </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-slate-900 rounded-2xl border border-slate-800 shadow-2xl ring-1 ring-white/5 overflow-hidden flex flex-col">
         <div className="bg-slate-900/50 backdrop-blur-md border-b border-slate-800/60 p-5 flex justify-between items-center">
           <div className="flex items-center gap-4">
             ...
             <div>
               <h2 className="text-base font-bold text-slate-100 tracking-wide uppercase">Materiais</h2>
               <div className="text-xs text-slate-500 font-medium mt-0.5">Estimativa de quantidades</div>
             </div>
          </div>
          
          <div className="flex items-center gap-2">
            <button onClick={() => setShowConfig(!showConfig)} type="button" className={`p-2.5 rounded-lg transition-all ${showConfig ? 'bg-amber-500/20 text-amber-500' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800'}`} title="Configurar Traço"><Settings size={20} /></button>
          </div>
        </div>

        {showConfig && (
          <div className="p-6 bg-slate-950/50 border-b border-slate-800 animate-in slide-in-from-top-2 duration-200">
            <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-4">Traço (por m³ de betão)</h3>
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div className="space-y-1.5">
                <label className="text-slate-500 font-medium">Cimento (kg)</label>
              ...
              <div className="space-y-1.5">
                <label className="text-slate-500 font-medium">Areia (m³)</label>
              ...
              <div className="space-y-1.5">
                <label className="text-slate-500 font-medium">Brita (m³)</label>
              ...
              <div className="space-y-1.5">
                <label className="text-slate-500 font-medium">Água (L)</label>
        ...
        <div className="p-6 space-y-4 flex-1 overflow-y-auto custom-scrollbar">
           <div className="flex items-center justify-between p-5 bg-gradient-to-r from-slate-800/40 to-slate-800/10 rounded-2xl border border-slate-800 transition-colors">
              <div className="flex items-center gap-4">
                 ...
                 <div><div className="font-bold text-slate-200 text-lg">Cimento</div><div className="text-xs text-slate-400 font-mono mt-1 font-medium">{bags} {units.cementBag}</div></div>
              </div>
           ...
           <div className="grid grid-cols-2 gap-4">
              <div className="p-5 bg-gradient-to-r from-amber-900/10 to-amber-900/5 rounded-2xl border border-amber-900/20">
                <div className="font-bold text-amber-200/80 text-sm mb-2">Areia <span className="text-[10px] text-amber-500/50 ml-1 font-normal uppercase">Lavada</span></div>
              ...
              <div className="p-5 bg-gradient-to-r from-stone-800/40 to-stone-800/10 rounded-2xl border border-slate-700/50">
                <div className="font-bold text-stone-300 text-sm mb-2">Brita <span className="text-[10px] text-stone-500 ml-1 font-normal uppercase">12/24</span></div>
           ...
           <div className="bg-gradient-to-br from-slate-800 to-slate-900 rounded-2xl border border-red-500/30 flex flex-col transition-all overflow-hidden relative group">
              ...
              <div className={`p-5 pl-7 flex justify-between items-start cursor-pointer ${showSteelDetails ? 'pb-2' : ''}`} onClick={() => setShowSteelDetails(!showSteelDetails)}>
                 <div className="flex items-center gap-4">
                    ...
                    <div>
                        <span className="text-base font-bold text-white tracking-wide">Aço (Armadura)</span>
                 ...
                 <div className="text-right">
                    ...
                    {isPro ? (
                        <div className="flex items-center justify-end gap-1 mt-2 text-xs text-slate-400">
                             Detalhes <ChevronDown size={14} className={`transition-transform ${showSteelDetails ? 'rotate-180' : ''}`} />
                        </div>
                    ) : (
                      ...
                      {isPro ? (
                          ...
                          <div className="relative overflow-hidden rounded-xl p-4 bg-slate-900/50 text-center mt-2">
                              <div className="opacity-20 blur-[2px] pointer-events-none select-none">
                                  <div className="flex justify-between text-sm mb-3"><span>Longitudinal Ø12</span><span>120 kg</span></div>
                              </div>
                              <div className="absolute inset-0 flex flex-col items-center justify-center bg-slate-950/40 z-10">
                                  <button onClick={onRequestUpgrade} type="button" className="bg-amber-600 hover:bg-amber-500 text-white text-xs font-bold py-2 px-4 rounded-full shadow-lg flex items-center gap-2 hover:scale-105 transition-transform"><Crown size={12} /> Ver Detalhe</button>
                              </div>
                          </div>
                      )}
                  </div>
           ...
           <div className="flex items-center justify-between p-5 bg-blue-950/10 rounded-2xl border border-blue-900/20">
              <div className="flex items-center gap-3"><div className="bg-blue-500/10 p-2 rounded-lg text-blue-400"><Droplets size={18}/></div><div className="font-bold text-slate-200 text-sm">Água</div></div>
        ...
        <div className="p-6 border-t border-slate-800 bg-slate-950/30">
          <div className="grid grid-cols-2 gap-4">
            ...
            <button 
              onClick={handleEmail}
              type="button"
              className={`flex items-center justify-center gap-3 py-3 rounded-xl text-sm font-bold border transition-all ${isPro ? 'bg-indigo-600 text-white border-indigo-500 hover:bg-indigo-500' : 'bg-slate-900 text-slate-500 border-slate-800 opacity-70 hover:opacity-100'}`}
            >
              {isPro ? <Mail size={18} /> : <Lock size={16} />}
              <span>Email</span>
            </button>
          </div>
        </div>
